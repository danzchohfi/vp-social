...
- **No backwards-compat shims, feature flags, or deprecated re-exports.** Schema migrations are versioned in `webapp/lib/db/migrations/`, but feature code can still rename/drop call sites freely as long as a paired migration is generated and committed. Don't keep "deprecated" exports around for safety — delete and update call sites in the same PR.
...
- **Schema edits are atomic with their migration.** Edit `lib/db/schema.ts`, then `npm run db:generate` to produce a numbered SQL file in `webapp/lib/db/migrations/`. Commit BOTH the schema edit and the generated SQL together. Vercel runs `npm run db:migrate` (non-interactive) on deploy. Pushing schema-only without the migration file = production crash on next query.
...
npm run db:generate    # Generate Drizzle migration .sql file from schema.ts
npm run db:migrate     # Apply pending migrations to Neon (used in CI; safe, non-interactive)
npm run db:push        # Push schema directly (interactive — for local dev only)
...
Vercel's `buildCommand` is `npm run db:migrate && npm run build` (see `vercel.json`). Schema changes flow:

1. Edit `lib/db/schema.ts` locally.
2. Run `npm run db:generate` — creates a numbered SQL file in `webapp/lib/db/migrations/`.
3. Inspect the generated SQL. If it looks right, commit `lib/db/migrations/` along with the schema change.
4. Push. Vercel runs `db:migrate` non-interactively, applying any pending migrations to Neon.

Why `migrate` and not `push`: `drizzle-kit push` prompts interactively whenever it sees a destructive op (data-loss warning, unique constraint on populated table). Vercel's CI has no TTY, so `push` aborted mid-deploy and the build proceeded with code referencing columns that hadn't been applied — production crashed (incident on 2026-05-06). `migrate` reads pre-generated SQL files and a journal, no prompts. The first baseline migration was seeded into `drizzle.__drizzle_migrations` via a one-time SQL after the migrate config switch.
...
| File | Role |
|---|---|
| `lib/db/schema.ts` | Single source of truth for every table. Edit + `npm run db:generate` + commit the new SQL in `lib/db/migrations/`. |
...
- `db:generate` reads schema.ts only — no env needed. `db:migrate` and `db:push` need `DATABASE_URL`: `set -a; source .env.local; set +a; npm run db:migrate` (xargs version mangles values with spaces).
